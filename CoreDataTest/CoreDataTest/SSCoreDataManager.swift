...
public class SSCoreDataManager: NSObject {
    ...
    public func insert<T: SSCoreDataFetchProtocol>(model: T) {
        
        let person = NSEntityDescription.insertNewObject(forEntityName: "Person", into: context) as! T
        
        
        let insertRequest = model.fetchRequest()
        do {
    ...
    public func delete<T>(model: T) {
        
    }
    
    public func query<T: SSCoreDataFetchProtocol>(model: T) -> [T]? {
        let queryRequest =  model.fetchRequest()
        do {
